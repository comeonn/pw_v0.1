...
|--------|------|------|
| `pending_pay` | 待支付 | 未支付 |
| `pending_dispatch` | 待派单 | 老板已下单未提交派单信息 / 或未确认派单 |
| `waiting_worker` | 待接单 | 进入接单大厅可见，等待打手接单 |
| `pending_close` | 待结单 | 打手已接单，等待打手服务结束后由老板确认 |
| `closed` | 已结单 | 老板确认结单 |
| `finished` | 已结束 | 管理员将没人接的单或需要取消的订单改为已结束 |

管理端「改为已结束」仅对 **待支付、待派单、待接单、待结单** 的订单显示；**已结单、已结束** 不再可改。
...
- **优先**：补齐订单全流程接口并前端改为真实 API（派单、接单、结单、管理员改为已结束等）
- 微信网页授权 + 角色区分（老板 / 打手）
- 真实下单流程与支付接口
- 钱包与提现的真实后台处理（含风控与审核）
...
6. **打手端业务接口**
   - 接单：`POST /api/worker/orders/:id/accept`（打手填写我的游戏 ID、预计上号、备注后接单）
   - 订单状态流转：待接单 / 待结单 / 已结单 / 已结束
   - 钱包：
     - 收入结算记录
     - 可提现余额查询
     - 提现申请接口（与客服 / 微信转账结合）
